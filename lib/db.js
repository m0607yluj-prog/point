...
async function withDb(mutator) {
  ...
  try {
    ...
    applyExpiredChorePenalties(data);
  ...
  for (const chore of db.chores) {
    ...
    for (let k = 0; k < elapsedPeriods; k++) {
      ...
      for (const child of targets) {
        ...
        const completedCount = db.choreLogs
          .filter((l) =>
...
// For each active adhoc chore/task past its deadline (dueAt), any target
// child (the assigned child, or everyone if unassigned) who hasn't reported
// it yet gets a synthetic "expired" chore log recording the late penalty —
// mirrors applyExpiredPenalties for questions, same lazy cron-free approach.
function applyExpiredChorePenalties(db) {
  const nowIso = new Date().toISOString();
  for (const chore of db.chores) {
    if (!chore.active || chore.type !== 'adhoc' || !chore.dueAt || chore.dueAt >= nowIso) continue;
    const targets = chore.assignedChildId ? db.children.filter((c) => c.id === chore.assignedChildId) : db.children;
    for (const child of targets) {
      const already = db.choreLogs.find((l) => l.choreId === chore.id && l.childId === child.id);
      if (already) continue;
      const penalty = Math.max(0, Number(chore.latePenalty) || 0);
      db.choreLogs.push({
        id: nextId(db.choreLogs),
        choreId: chore.id,
        choreName: chore.name,
